...
void BoardUI::draw(SDL_Renderer* ren) const
{
    ...
    if (drawing_wire_ && o_mouse_tile)
        draw_temporary_wire(ren, drawing_wire_->first, o_mouse_tile->pos, drawing_wire_->second);
...
{
    auto tiles = BoardSpriteSheet::board_tiles();
...
{
    draw_icon(ren, BoardSpriteSheet::board_tiles().inner, x, y);

    auto it = board_->components.find({ x, y });
    if (it != board_->components.end())
        for (BoardSpriteSheet::Sprite const& sprite: BoardSpriteSheet::sprite(it->second))
            draw_icon(ren, sprite, x, y);
}

void BoardUI::draw_icon(SDL_Renderer* ren, BoardSpriteSheet::Sprite icon, ssize_t x, ssize_t y) const
{
...
void BoardUI::draw_temporary_wire(SDL_Renderer* ren, Position const& start, Position const& end, WireType wire_type) const
{
